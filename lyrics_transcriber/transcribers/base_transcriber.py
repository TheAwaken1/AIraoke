...
class BaseTranscriber(ABC):
    ...
    def _save_to_cache(self, cache_path: str, raw_data: Any) -> None:
        """Save raw API response data to cache."""
        self.logger.debug(f"Saving JSON to cache: {cache_path}")
        # Ensure raw_data is a dict; convert TranscriptionData if needed
        data_to_save = raw_data.to_dict() if hasattr(raw_data, 'to_dict') else raw_data
