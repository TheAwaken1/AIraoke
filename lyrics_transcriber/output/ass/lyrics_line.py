...
class LyricsLine:
    ...
    def _get_font(self, style: Style) -> ImageFont.FreeTypeFont:
        ...
        # Parse Fontsize as a string and convert to integer
        font_size = int(style.Fontsize)  # Parse the string to an integer
        # Scale down the font size to match ASS rendering
        adjusted_size = int(font_size * ASS_FONT_SCALE)
        self.logger.debug(f"Adjusting font size from {font_size} to {adjusted_size} to match ASS rendering")
