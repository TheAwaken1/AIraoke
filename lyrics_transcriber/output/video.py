...
class VideoGenerator:
    ...
    def generate_video(self, ass_path: str, audio_path: str, output_prefix: str) -> str:
        ...
        output_path = self._get_output_path(f"{output_prefix} (With Vocals)", "mp4") # <--- Change extension here
    ...
    def _build_ffmpeg_command(self, ass_path: str, audio_path: str, output_path: str) -> List[str]:
        ...
        vf_argument = f"ass='{escaped_ass_path}'"

        cmd.extend([
            "-i", audio_path,
            "-c:a", "aac", # Keep audio codec as AAC
            "-b:a", "192k", # Keep audio bitrate
            "-vf", vf_argument, # Keep escaped/quoted path
            "-c:v", self._get_video_codec(), # Should be libx264

            # --- ADD THESE LINES for compatibility ---
            "-profile:v", "baseline", # Use baseline profile
            "-level", "3.0",         # Use level 3.0
            "-pix_fmt", "yuv420p",   # Common pixel format
            # --- END ADDITION ---

            # Video quality settings (keep or adjust if needed)
            "-preset", "fast",
            "-b:v", "5000k",
            "-minrate", "5000k",
            "-maxrate", "20000k",
