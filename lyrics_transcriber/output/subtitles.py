...
class SubtitlesGenerator:
    ...
    def __init__(
        ...
        Args:
            ...
            font_size: Font size for subtitles (as integer for calculations)
        ...
        self.font_size = font_size  # Integer for calculations
    ...
    def _create_styled_ass_instance(self, resolution, fontsize):
        ...
        # Set all numerical properties as strings for the ASS library
        style.Fontsize = self.styles["karaoke"]["font_size"]  # Keep as string
        ...
        # Set numerical properties as strings
        style.ScaleX = self.styles["karaoke"]["scale_x"]
        style.ScaleY = self.styles["karaoke"]["scale_y"]
        style.Spacing = self.styles["karaoke"]["spacing"]
        style.Angle = self.styles["karaoke"]["angle"]
        style.BorderStyle = self.styles["karaoke"]["border_style"]
        style.Outline = self.styles["karaoke"]["outline"]
        style.Shadow = self.styles["karaoke"]["shadow"]
        style.MarginL = self.styles["karaoke"]["margin_l"]
        style.MarginR = self.styles["karaoke"]["margin_r"]
        style.MarginV = self.styles["karaoke"]["margin_v"]
        style.Encoding = self.styles["karaoke"]["encoding"]

        # Log the style values for debugging
        self.logger.debug(f"Style values before adding to ASS: {style.__dict__}")

        a.add_style(style)

        # Log the style values after adding to ASS to see if Fontsize is being set to None
        self.logger.debug(f"Style values after adding to ASS: {style.__dict__}")
